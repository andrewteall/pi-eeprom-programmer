    ...
    /**
     * @brief GPIO_CHIP struct to hold gpio chip configuration parameters.
     */
    struct GPIO_CHIP{
        char *chipname;
        int numGPIOLines;
     ...
     * @param *gpioChip A pointer to the GPIO_CHIP struct to reference gpio chip to be used.
     ...
     * @param *sOptions A pointer to the OPTIONS struct to reference all the configured options.
     * @param byteWriteCounter A pointer to an int to keep track of the number of bytes written.
     * @return int Returns 0 if successful 1 if error.
     */
    int writeByteToAddress(struct GPIO_CHIP* gpioChip, 
